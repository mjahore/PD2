...
c
c BELOW ARE ROUTINES FOR BINARY CHECKPOINT FILES FOR SPEED/STORAGE
c
...
c                                   (BINARY FILE)
c
      subroutine bf_write_checkpoint_master_level_bin()
      implicit none
      include 'mpif.h'
      include 'common.f'
      include 'Binary_Fluid/binary_fluid_var.f'
      integer i,k,j, l,clust_idx
      integer recv_count, ierr
      double precision r_time1, r_time2
      ...
      if (cluster_rank .eq. MASTER) then
         ...
         write(*,*) ' '
         write(*,*) 'Writing checkpoint at master level...'
         open (12, file='checkpoint.data', access='STREAM')
         call FSEEK(12, 0, 0, ierr)
         do i=1, cluster_size*n_poly
            write(12) p_species(i)
         enddo
         write(12) itime
         ...
         do i=1, local_no
            ...
            write(12) poly_x(i), poly_y(i), poly_z(i)
            write(12) fx(i), fy(i), fz(i)
            write(12) vx(i), vy(i), vz(i)
            write(12) dble(logical_id(i)), dble(species(i)),
     >                   dble(p_flag(i))
         enddo
      endif
     ...
     >                    MPI_COMM_WORLD, mpi_status(10), mpi_err)
            ...
            do j=1, recv_count
               ...
               write(12) DBL_BUF(k+1),  DBL_BUF(k+2),  DBL_BUF(k+3)
               write(12) DBL_BUF(k+4),  DBL_BUF(k+5),  DBL_BUF(k+6)
               write(12) DBL_BUF(k+7),  DBL_BUF(k+8),  DBL_BUF(k+9)
               write(12) DBL_BUF(k+10), DBL_BUF(k+11), DBL_BUF(k+12)
               write(12) DBL_BUF(k+13), DBL_BUF(k+14), DBL_BUF(k+15)
            enddo
         endif
      enddo
...
c
c bf_read_checkpoint_master_level_bin() - Restores a simulation from the master level. (BINARY FILE)
c
      subroutine bf_read_checkpoint_master_level_bin()
      implicit none
      include 'mpif.h'
      include 'common.f'
      include 'Binary_Fluid/binary_fluid_var.f'
      ...
      if (cluster_rank .eq. MASTER) then
         ...
         do i=1, local_no
            read(12) x(i), y(i), z(i)
            read(12) poly_x(i), poly_y(i), poly_z(i)
            read(12) fx(i), fy(i), fz(i)
            read(12) vx(i), vy(i), vz(i)
            read(12) db1, db2, db3
            logical_id(i) = int(db1)
            species(i)    = int(db2)
            p_flag(i)     = int(db3)
         enddo
      endif
     ...
     >                 ,i
            ...
            do j=1, local_no_tmp
               k = (j-1)*15
               read(12) DBL_BUF(k+1),  DBL_BUF(k+2),  DBL_BUF(k+3) 
               read(12) DBL_BUF(k+4),  DBL_BUF(k+5),  DBL_BUF(k+6) 
               read(12) DBL_BUF(k+7),  DBL_BUF(k+8),  DBL_BUF(k+9) 
               read(12) DBL_BUF(k+10), DBL_BUF(k+11), DBL_BUF(k+12)
               read(12) DBL_BUF(k+13), DBL_BUF(k+14), DBL_BUF(k+15)
            enddo
